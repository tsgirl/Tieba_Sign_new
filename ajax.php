...
switch($_GET['v']){
	...
	case 'get-bind-status':
		...
		if(isset($data['data'])&&isset($data['data']['user_portrait'])) dsetcookie("avatar_{$uid}", 'http://tb.himg.baidu.com/sys/portrait/item/'.$data['data']['user_portrait']);
		break;
	case 'get-setting':
		$data = get_setting($uid);
		if($data['stoken']) $data['stoken']=base64_decode($data['stoken']);
		if($data['autoagree']) $data['autoagree']=base64_decode($data['autoagree']);
		break;
	case 'sign-log':
		$date = date('Ymd');
